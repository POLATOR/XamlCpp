...
struct xaml_dynamic_type_info_impl : xaml_reflection_info_implement<xaml_dynamic_type_info_impl, xaml_dynamic_type_info>
{
    ...
    xaml_result XAML_CALL get_property(xaml_string * name, xaml_property_info ** ptr) noexcept override
    {
        ...
        if (result != XAML_S_OK) {
            ...
            XAML_RETURN_IF_FAILED(xaml_event_info_new(
                event_name,
                // adder
                [=](xaml_object * object, xaml_method_info * method, int32_t * handle) -> xaml_result {
                    ...
                    event->add(
                        [method = xaml_ptr<xaml_method_info>{method}](xaml_interface_t<xaml_object> sender,
                                                                      xaml_interface_t<xaml_string> e) noexcept -> xaml_result {
            ...
            xaml_property_info_new(
                name, xaml_guid_xaml_string,
                // getter
                ...
                &prop_info);
            is_inserted = false;
            XAML_RETURN_IF_FAILED(m_prop_map->insert(name, prop_info, &is_inserted));
            XAML_RETURN_IF_FAILED(prop_info->query(ptr));
        }
        return XAML_S_OK;
    }
...
EXTERN_C xaml_result XAML_CALL xaml_dynamic_type_info_new(xaml_guid XAML_CONST_REF type, xaml_string * name, xaml_string * include_file,
                                                          xaml_dynamic_type_info ** ptr) XAML_NOEXCEPT
{
