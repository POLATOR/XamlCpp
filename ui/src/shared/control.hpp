...
#elif defined(XAML_UI_QT)
    XAML_PROP_IMPL(handle, QPointer<QWidget>, QPointer<QWidget> *, QPointer<QWidget>)

    template <typename T, typename... Args>
    xaml_result XAML_CALL create(Args &&... args) noexcept
    {
        QPointer<QWidget> handle = nullptr;
...
{
    xaml_result XAML_CALL get_handle(HWND * pvalue) noexcept override
    {
        return this->m_outer->get_handle(pvalue);
    }
    xaml_result XAML_CALL set_handle(HWND value) noexcept override
    {
        return this->m_outer->set_handle(value);
    }
    xaml_result XAML_CALL wnd_proc(xaml_win32_window_message const & msg, LPARAM * res) noexcept override
    {
        return this->m_outer->wnd_proc(msg, res);
    }
    xaml_result XAML_CALL get_real_size(xaml_size * pvalue) noexcept override
    {
        return this->m_outer->get_real_size(pvalue);
    }
    xaml_result XAML_CALL set_real_size(xaml_size const & value) noexcept override
    {
        return this->m_outer->set_real_size(value);
    }
    xaml_result XAML_CALL get_real_margin(xaml_margin * pvalue) noexcept override
    {
        return this->m_outer->get_real_margin(pvalue);
    }
    xaml_result XAML_CALL set_real_margin(xaml_margin const & value) noexcept override
    {
        return this->m_outer->set_real_margin(value);
    }
};
#elif defined(XAML_UI_COCOA)
template <typename T2, typename D, typename Base2>
...
{
    xaml_result XAML_CALL get_handle(OBJC_OBJECT(NSView) * pvalue) noexcept override
    {
        return this->m_outer->get_handle(pvalue);
    }
    xaml_result XAML_CALL set_handle(OBJC_OBJECT(NSView) value) noexcept override
    {
        return this->m_outer->set_handle(value);
    }
    xaml_result XAML_CALL get_delegate(OBJC_OBJECT(XamlDelegate) * pvalue) noexcept override
    {
        return this->m_outer->get_delegate(pvalue);
    }
    xaml_result XAML_CALL set_delegate(OBJC_OBJECT(XamlDelegate) value) noexcept override
    {
        return this->m_outer->set_delegate(value);
    }
};
#elif defined(XAML_UI_GTK3)
template <typename T2, typename D, typename Base2>
...
{
    xaml_result XAML_CALL get_handle(GtkWidget ** pvalue) noexcept override
    {
        return this->m_outer->get_handle(pvalue);
    }
    xaml_result XAML_CALL set_handle(GtkWidget * value) noexcept override
    {
        return this->m_outer->set_handle(value);
    }
};
...
{
    xaml_result XAML_CALL get_handle(QPointer<QWidget> * pvalue) noexcept override
    {
        return this->m_outer->get_handle(pvalue);
    }
    xaml_result XAML_CALL set_handle(QPointer<QWidget> value) noexcept override
    {
        return this->m_outer->set_handle(value);
    }
};
...
struct xaml_control_implement : xaml_weak_implement<T, Base>
{
    ...
    xaml_result XAML_CALL add_resource(xaml_string * key, xaml_object * value) noexcept override
    {
        return m_internal.add_resource(key, value);
    }
    xaml_result XAML_CALL get_resource(xaml_string * key, xaml_object ** pvalue) noexcept override
    {
        return m_internal.get_resource(key, pvalue);
    }
    xaml_result XAML_CALL get_resources(xaml_map_view<xaml_string, xaml_object> ** ptr) noexcept override
    {
        return m_internal.get_resources(ptr);
    }
    ...
    xaml_result XAML_CALL wnd_proc(xaml_win32_window_message const & msg, LRESULT * presult) noexcept
    {
        return m_internal.wnd_proc(msg, presult);
    }
#elif defined(XAML_UI_COCOA)
...
#elif defined(XAML_UI_QT)
    XAML_PROP_INTERNAL_IMPL(handle, QPointer<QWidget> *, QPointer<QWidget>)
