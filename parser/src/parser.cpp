...
struct parser_impl
{
    ...
    {
        return cfile_istream_invoke<char>(
            [this](istream & stream) noexcept -> xaml_result {
...
xaml_result parser_impl::parse_markup(string_view value, xaml_markup_node ** ptr) noexcept
{
    ...
    while (i < value.length()) {
        ...
        if (can_write) {
            ...
            if (!prop_value.empty() && prop_value.front() == '{' && prop_value.back() == '}') {
...
static constexpr string_view x_ns{"https://polator.ru/schemas/vpl-1/xaml/"};

// Add string node to properties vector
static xaml_result props_add_string_property(xaml_ptr<xaml_vector<xaml_attribute_property>> const & props, xaml_ptr<xaml_type_info> const & t,
                                             xaml_ptr<xaml_property_info> const & prop, string_view value) noexcept
{
...
static xaml_result get_cprop_values(xaml_ptr<xaml_map<xaml_string, xaml_attribute_collection_property>> const & cprops,
                                    xaml_ptr<xaml_collection_property_info> const & cprop, xaml_ptr<xaml_type_info> const & type,
                                    xaml_vector<xaml_node> ** pvalues) noexcept
{
                    ...
                    if (dm_index != string_view::npos) {
                        ...
                        if (XAML_SUCCEEDED(type->get_property(attr_name_str, &prop))) {
                            ...
                            if (can_write) {
                                ...
                                if (!attr_value.empty() && attr_value.front() == '{' && attr_value.back() == '}') {
                                    xaml_ptr<xaml_markup_node> ex;
...
xaml_result parser_impl::parse(xaml_node ** ptr) noexcept
{
    ...
    auto it = find_if(root_nodes.begin(), root_nodes.end(), [](xml_node & node) noexcept {
...
static xaml_result XAML_CALL xaml_parse_parse_impl(parser_impl & parser, xaml_meta_context * ctx, xaml_node ** ptr,
                                                   xaml_vector_view<xaml_string> ** pheaders) noexcept
{
    parser.ctx = ctx;
...
xaml_result XAML_CALL xaml_parser_parse_buffer(xaml_meta_context * ctx, xaml_buffer * buffer, xaml_node ** ptr,
                                               xaml_vector_view<xaml_string> ** pheaders) noexcept
{
    parser_impl parser{};
    XAML_RETURN_IF_FAILED(parser.init());
