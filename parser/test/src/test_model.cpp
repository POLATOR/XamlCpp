...
struct xaml_test_model_impl : xaml_weak_implement<xaml_test_model_impl, xaml_test_model>
{
    ...
    XAML_EVENT_IMPL(diagramData_changed, xaml_object, xaml_string)
    XAML_PROP_STRING_EVENT_IMPL(diagramData)
    ...
    xaml_result XAML_CALL init() noexcept
    {
        ...
        XAML_RETURN_IF_FAILED(xaml_event_new(&m_diagramData_changed));
...
xaml_result XAML_CALL xaml_test_model_register(xaml_meta_context* ctx) noexcept
{
    ...
    XAML_TYPE_INFO_ADD_PROP_EVENT(diagramData, xaml_string);
